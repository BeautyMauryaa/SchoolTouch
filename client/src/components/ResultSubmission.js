...
export default function ResultSubmission() {
  ...
  const fetchData = async () => {
    try {
      const [examsRes, classesRes, subjectsRes, teachersRes] =
        await Promise.all([
    ...
    const fetchStudentsForExam = async () => {
      ...
      try {
        ...
        const studentsRes = await axios.get(
          `${API_BASE_URL}/students?classId=${exam.class._id}`,
          { headers },
        );
  ...
  const handleMarkChange = (studentId, value) => {
    ...
    setMarks((prev) => ({
      ...prev,
      [studentId]: value === "" ? "" : Math.max(0, parseInt(value, 10) || 0),
    }));
  };
  ...
  const handleSubmitMarks = async (e) => {
    ...
    const subjectDetails = selectedExamDetails?.subjects?.find(
      (s) => s.subjectId._id === selectedSubject,
    );
  ...
  const selectedExamDetails = exams.find((e) => e._id === selectedExam);
  const maxMarks =
    selectedExamDetails?.subjects?.find(
      (s) => s.subjectId._id === selectedSubject,
    )?.maxMarks || 0;
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <button
        ...
        className="bg-orange-600 text-white px-4 py-2 rounded-md mb-4"
      >
          ...
          >
            ...
            <div className="md:col-span-3">
              ...
              <input
                type="text"
                name="examName"
                value={unitTestFormData.examName}
            ...
            <div>
              ...
              <select
                name="classId"
                value={unitTestFormData.classId}
                onChange={handleUnitTestChange}
                className="w-full mt-1 rounded-md"
                required
              >
            ...
            <div>
              ...
              <select
                name="subjectId"
                value={unitTestFormData.subjectId}
                onChange={handleUnitTestChange}
                className="w-full mt-1 rounded-md"
                required
              >
            ...
            <div>
              ...
              <input
                type="number"
                name="maxMarks"
                value={unitTestFormData.maxMarks}
                onChange={handleUnitTestChange}
                className="w-full mt-1 rounded-md"
                required
              />
            ...
            <div>
              ...
              <input
                type="number"
                name="passingMarks"
                value={unitTestFormData.passingMarks}
                onChange={handleUnitTestChange}
                className="w-full mt-1 rounded-md"
                required
              />
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        <h3 className="text-xl font-bold mb-4">
          Enter Marks for Existing Exams
        </h3>
