...
export default function LoginPage() {
  const [isFirstAdminCreated, setIsFirstAdminCreated] = useState(false);
  ...
  useEffect(() => {
    const checkFirstAdmin = async () => {
      try {
        ...
        setIsFirstAdminCreated(false); // show signup if API unreachable
        setIsLoading(false);
      }
    };
    checkFirstAdmin();
    ...
    if (role === "parent") {
      try {
        if (step === "login") {
          ...
          await axios.post(`${API_BASE}/auth/login/parent/request-otp`, {
            phone,
          });
          setStep("otp");
        ...
        if (step === "otp") {
          ...
          const res = await axios.post(
            `${API_BASE}/auth/login/parent/verify-otp`,
            { phone, otp },
          );
          ...
          {role !== "parent" && (
            <>
              ...
              <input
                ...
                placeholder={
                  role === "student" ? "Enter Student Code" : "Enter Email"
                }
                value={identifier}
