      ...
      try {
        ...
        if (userRole === "student") {
          ...
          const parentRes = await axios.get(
            `${API_BASE_URL}/parents/${profileId}`,
            { headers },
          );
        ...
        if (!classIdToFetch) {
          setError(
            "Class not found for your profile. Please check your admin assignment.",
          );
          setLoading(false);
          return;
        }
        ...
        const classDetailsRes = await axios.get(
          `${API_BASE_URL}/classes/${classIdToFetch}`,
          { headers },
        );
        ...
        setError(
          err.response?.data?.message ||
            "Failed to fetch class data. Server error.",
        );
        setLoading(false);
      }
    };
