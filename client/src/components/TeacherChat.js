...
export default function TeacherChat() {
    ...
    const fetchParents = async () => {
      try {
        const res = await axios.get(
          `${API_BASE_URL}/chat/parents/${teacherId}`,
          { headers },
        );
        setParents(res.data);
        setLoading(false);
  ...
  const handleSelectParent = async (parent) => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/chat/history/${teacherId}/${parent._id}`,
        { headers },
      );
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {messages.map((msg, index) => (
                ...
                >
                  <div
                    className={`rounded-lg p-2 max-w-sm ${msg.sender.toString() === teacherId ? "bg-blue-500 text-white" : "bg-gray-300"}`}
                  >
                    {msg.messageText}
                  </div>
                </div>
