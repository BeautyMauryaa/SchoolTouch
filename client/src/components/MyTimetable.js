      ...
      try {
        ...
        if (role === "student" || role === "parent") {
          const studentRes = await axios.get(
            `${API_BASE_URL}/students/${profileId}`,
            { headers },
          );
          const classId = studentRes.data.class._id;
          res = await axios.get(`${API_BASE_URL}/timetables/class/${classId}`, {
            headers,
          });
        } else if (role === "teacher") {
          const teacherRes = await axios.get(
            `${API_BASE_URL}/teachers/${profileId}`,
            { headers },
          );
          const assignedClassId = teacherRes.data.assignedClass;
          if (assignedClassId) {
            res = await axios.get(
              `${API_BASE_URL}/timetables/class/${assignedClassId._id}`,
              { headers },
            );
          } else {
            res = await axios.get(
              `${API_BASE_URL}/timetables/teacher/${profileId}`,
              { headers },
            );
          }
        }
  ...
  return (
    <div className="container mx-auto p-4">
      <h2 className="text-2xl font-bold mb-4">
        My Timetable for {timetable.class.className} {timetable.class.section}
      </h2>
