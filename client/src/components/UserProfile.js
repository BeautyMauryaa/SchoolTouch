...
export default function UserProfile() {
  ...
  if (!profile)
    return <p className="text-center p-8">Profile data not available.</p>;
  ...
  const renderDetails = () => {
    ...
    if (role === "teacher") {
      data.push({
        ...
        value:
          `${details.assignedClass?.className} ${details.assignedClass?.section || ""}` ||
          "N/A",
        icon: "FaSchool",
      });
      data.push({
        label: "Subjects",
        value:
          details.assignedSubjects?.map((s) => s.subjectName).join(", ") ||
          "N/A",
        icon: "FaBook",
      });
    }
    ...
    if (role === "student") {
      data.push({
        label: "Class & Section",
        value:
          `${details.class?.className} ${details.class?.section || ""}` ||
          "N/A",
        icon: "FaSchool",
      });
      data.push({
        label: "Roll Number",
        value: details.rollNumber || "N/A",
        icon: "FaIdCard",
      });
      data.push({
        label: "Parent Contact",
        value: `${details.parent?.name} (${details.parent?.phone})` || "N/A",
        icon: "FaUsers",
      });
      data.push({
        label: "Attendance Avg",
        value: profile.analytics?.attendance || "N/A",
        icon: "FaCheckCircle",
      });
    }
    ...
    if (role === "parent") {
      data.push({
        ...
        value:
          details.children
            ?.map((c) => `${c.name} (${c.class?.className || "N/A"})`)
            .join("; ") || "N/A",
        icon: "FaChild",
      });
    }
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl">
        <div className="flex justify-between items-start mb-6 border-b pb-4">
          <h3 className="text-xl font-semibold">
            {profile.details.name || profile.baseInfo.name} -{" "}
            {profile.baseInfo.role.toUpperCase()}
          </h3>
          <button
