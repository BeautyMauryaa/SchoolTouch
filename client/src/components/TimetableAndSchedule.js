  ...
  const fetchTimetableAndSchedule = async () => {
    try {
      const timetableRes = await axios.get(
        `${API_BASE_URL}/timetables/class/${selectedClassId}`,
        { headers },
      );
      setTimetable(timetableRes.data.schedule);
      const schedulesRes = await axios.get(
        `${API_BASE_URL}/schedules/class/${selectedClassId}`,
        { headers },
      );
  ...
  const handleSubmitTimetable = async () => {
    try {
      await axios.post(
        ...
        { classId: selectedClassId, schedule: timetable },
        { headers },
      );
  ...
  const handleSubmitSchedule = async (e) => {
    ...
    try {
      await axios.post(
        ...
        { ...scheduleFormData, classId: selectedClassId },
        { headers },
      );
            ...
            {timetable.map((day, dayIndex) => (
              ...
              >
                ...
                {userRole === "admin" && (
                  ...
                  >
                    <FaPlus className="inline-block" /> Add Period
                  </button>
                )}
              </div>
            ))}
          ...
          <div>
            ...
            {userRole === "admin" && (
              <form
                ...
                className="bg-white p-6 rounded-lg shadow-md mb-4 grid grid-cols-1 gap-4"
              >
                <div>
                  <label>Title</label>
                  <input
