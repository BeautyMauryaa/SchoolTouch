    ...
    const fetchExamTimetable = async () => {
      ...
      try {
        const res = await axios.get(
          `${API_BASE_URL}/exams/timetable/student/${studentProfileId}`,
          { headers },
        );
        setExams(res.data);
      } catch (err) {
        setError(
          err.response?.data?.message || "Failed to fetch exam timetable.",
        );
      } finally {
        setLoading(false);
      }
                ...
                {exam.subjects.map((subject, index) => (
                  <tr key={index}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {subject.subjectId?.subjectName || "N/A"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {subject.date
