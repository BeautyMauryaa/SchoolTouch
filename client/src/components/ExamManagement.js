...
export default function ExamManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    const missingSchedule = scheduleData.find(
      (s) => !s.date || !s.room || !s.invigilator,
    );
    if (missingSchedule) {
      return setMessage(
        "Error: Please set Date, Room, and Invigilator for all subjects.",
      );
    }
    ...
    try {
      if (isEditing) {
        await axios.put(`${API_BASE_URL}/exams/${editId}`, finalPayload, {
          headers,
        });
      } else {
  ...
  const handleEdit = (exam) => {
    ...
    const filteredSubjects = allSubjects.filter(
      (subject) => subject.class._id === exam.class?._id,
    );
    setSubjectsForSelectedClass(filteredSubjects);
    ...
    const existingSchedule = exam.subjects.map((s) => ({
      ...
      subjectName:
        ...
        filteredSubjects.find((fs) => fs._id === s.subjectId?._id)
          ?.subjectName ||
        "N/A",
        ...
        {scheduleData.length > 0 && (
          <div className="md:col-span-2 mt-4">
            <h4 className="font-semibold text-lg mb-3 border-b pb-2">
              Set Subject Schedule & Invigilator
            </h4>
