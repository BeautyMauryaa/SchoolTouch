...
export default function HomeworkAndMaterials() {
  ...
  const handleSubmit = async (e) => {
    ...
    const fileFieldName =
      currentTab === "homework" ? "attachments" : "attachments"; // Backend should accept 'attachments' for both
  ...
  const submitStudentHomework = async (homeworkId) => {
    ...
    fileInput.onchange = async (e) => {
      ...
      try {
        await axios.post(
          `${API_BASE_URL}/homework/${homeworkId}/submit`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        <>
          ...
          {item.attachments.length > 0 && (
            <a
              ...
              download
              className="bg-gray-500 text-white px-3 py-1 rounded-md text-sm mr-2"
            >
              <FaDownload /> File
            </a>
          )}
          ...
          {userRole === "student" && (
            <button
              onClick={() => submitStudentHomework(item._id)}
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex space-x-4 mb-4">
        <button
          ...
          className={`py-2 px-4 rounded-md ${currentTab === "homework" ? "bg-blue-600 text-white" : "bg-gray-200"}`}
        >
          Homework
        </button>
        <button
          onClick={() => setCurrentTab("studyMaterial")}
          className={`py-2 px-4 rounded-md ${currentTab === "studyMaterial" ? "bg-blue-600 text-white" : "bg-gray-200"}`}
        >
          Study Material
        </button>
        {userRole === "teacher" && (
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          <h3 className="text-xl font-bold mb-4">
            Add New {currentTab === "homework" ? "Homework" : "Study Material"}
          </h3>
          <form
