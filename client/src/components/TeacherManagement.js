  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/teachers?search=${searchQuery}&class=${filterClass}`,
        { headers },
      );
    ...
    if (newSubject.classId && newSubject.subjectId) {
      setFormData((prev) => ({
        ...
        assignedSubjects: [
          ...
          { class: newSubject.classId, subject: newSubject.subjectId },
        ],
      }));
      setNewSubject({ classId: "", subjectId: "" });
    }
          ...
          <div className="md:col-span-2">
            ...
            <ul className="mt-2 list-disc ml-4">
              {formData.assignedSubjects?.map((s, index) => (
                ...
                >
                  <span>
                    {classes.find((c) => c._id === s.class)?.className}{" "}
                    {classes.find((c) => c._id === s.class)?.section} -{" "}
                    {subjects.find((sub) => sub._id === s.subject)?.subjectName}
                  </span>
                  <button
                    type="button"
                    onClick={() => removeAssignedSubject(index)}
              ...
              <tr key={teacher._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {teacher.isClassTeacher
                    ? `${teacher.assignedClass?.className} ${teacher.assignedClass?.section}`
                    : "No"}
                </td>
