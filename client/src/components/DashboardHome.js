...
export default function DashboardHome() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const summaryRes = await axios.get(`${API_BASE_URL}/fees/summary`, {
          headers,
        });
        ...
        const fetchedStats = [
          {
            title: "Total Classes",
            value: summaryRes.data.totalClasses,
            icon: FaSchool,
            color: "bg-purple-100 text-purple-800",
          },
          {
            title: "Total Teachers",
            value: summaryRes.data.totalTeachers,
            icon: FaChalkboardTeacher,
            color: "bg-green-100 text-green-800",
          },
          {
            title: "Total Students",
            value: summaryRes.data.totalStudents,
            icon: FaUserGraduate,
            color: "bg-blue-100 text-blue-800",
          },
          {
            title: "Active Parents",
            value: summaryRes.data.totalParents,
            icon: FaUsers,
            color: "bg-yellow-100 text-yellow-800",
          },
          {
            title: "Fees Collected",
            value: `₹${(summaryRes.data.totalAmountPaid || 0).toFixed(2)}`,
            icon: FaMoneyBillWave,
            color: "bg-teal-100 text-teal-800",
          },
          {
            title: "Fees Pending",
            value: `₹${(summaryRes.data.totalPendingAmount || 0).toFixed(2)}`,
            icon: FaMoneyBillWave,
  ...
  const quickLinks = [
    {
      name: "Manage Classes",
      link: "/admin/dashboard/myclasses",
      color: "bg-orange-500",
    },
    {
    ...
    {
      name: "View Reports",
      link: "/admin/dashboard/results/view",
      color: "bg-blue-500",
    },
    {
  ...
  if (loading)
    return <p className="text-center p-8">Loading School Overview...</p>;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {quickLinks.map((link) => (
          <a
            key={link.name}
            href={link.link}
            className={`p-4 rounded-lg shadow-md text-white font-semibold text-center ${link.color} hover:shadow-xl transition duration-200`}
          >
            {link.name}
          </a>
        ))}
