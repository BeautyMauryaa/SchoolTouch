...
export default function HallTickets() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      await axios.post(
        `${API_BASE_URL}/exams/${selectedExamId}/halltickets`,
        {},
        { headers },
      );
      ...
      setMessage(
        err.response?.data?.message || "Failed to generate hall tickets.",
      );
    }
  };
  ...
  if (exams.length === 0)
    return <p>No exams found to generate hall tickets for.</p>;
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        >
          <FaDownload className="inline-block mr-2" /> Generate & Save Hall
          Tickets
        </button>
