...
export default function ClassManagementPage() {
  ...
  const fetchData = async () => {
    try {
      const [classesRes, teachersRes, studentsRes, subjectsRes] =
        await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`${API_BASE_URL}/classes/${editId}`, formData, {
          headers,
        });
      } else {
      ...
      >
        ...
        <div className="md:col-span-2">
          ...
          <select
            ...
            className="w-full mt-1 rounded-md h-24"
          >
            {subjects.map((s) => (
        ...
        <div className="md:col-span-2">
          ...
          <select
            ...
            onChange={handleMultiSelect}
            className="w-full mt-1 rounded-md h-40"
          >
            {filteredStudents.map((s) => (
              <option
                ...
                style={{
                  backgroundColor: formData.studentIds.includes(s._id)
                    ? "#ffd2bd"
                    : "white",
                }}
              >
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {classes.map((classItem) => (
              <tr key={classItem._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {classItem.subjects?.map((s) => s.subjectName).join(", ")}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
