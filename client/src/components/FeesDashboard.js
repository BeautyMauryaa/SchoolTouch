...
export default function FeesDashboard() {
  ...
  useEffect(() => {
    const fetchFees = async () => {
      try {
        let studentId;
        const parentRes = await axios.get(`${API_BASE}/parents/${profileId}`, {
          headers,
        });
        ...
        const res = await axios.get(`${API_BASE}/fees/child/${studentId}`, {
          headers,
        });
  ...
  if (!feeDetails || feeDetails.length === 0)
    return <p>No fee information available.</p>;
        ...
        {feeDetails.map((fee, index) => (
          <div key={index} className="bg-white p-6 rounded-lg shadow-md">
            ...
            <p className="mt-2 font-semibold">
              Status:
              <span
                className={`ml-2 px-2 py-1 rounded-full text-white ${fee.status === "Paid" ? "bg-green-500" : "bg-red-500"}`}
              >
                {fee.status}
              </span>
            </p>
      ...
      {showPayment && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <img
              src="/uploads/payment/upi_qr.png"
              alt="UPI QR Code"
              className="mx-auto mb-4 w-48 h-48"
            />
            <p className="mb-2">Scan QR or click link below:</p>
            <a
              href="upi://pay?pa=school@upi&pn=SchoolName&am=Amount&cu=INR"
              target="_blank"
