...
export default function AdminManageParents() {
  ...
  const handleSearch = async () => {
    ...
    const headers = { Authorization: `Bearer ${token}` };
    try {
      const res = await axios.get(`${API_BASE}/parents?search=${searchQuery}`, {
        headers,
      });
      setParents(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`${API_BASE}/parents/${editId}`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        await axios.post(`${API_BASE}/parents`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      setFormData({
